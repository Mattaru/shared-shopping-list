...
const handleRequest = async (request) => {
    ...
    else if (url.pathname.match("/admin/[0-9]+/delete") && request.method === "POST") 
        return await adminController.deleteList(request);
    else if (url.pathname === "/admin" && request.method === "GET") 
        return await adminController.viewAdmin(request);
